...
unsafe impl crate::Impl for StdCriticalSection {
    ...
    // true indicates that we are executing inside a critical section.
    unsafe fn get_states() -> (bool, bool) {
        println!("get_states called");
        (false, true)
    }

    unsafe fn get_state() -> bool {
        let state = IS_LOCKED.with(|l| l.get());
        println!("get_state called: {}", state);
        state
    }

    unsafe fn set_state(raw_restore_state: bool) {
        IS_LOCKED.with(|l| l.set(raw_restore_state));
        println!("set_state called: {}", raw_restore_state);
    }
}
